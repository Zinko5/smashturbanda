...
- **WebRTC Networking**: The game relies on P2P DataChannels via PeerJS. When attaching debug/monitoring listeners to the underlying `RTCPeerConnection`, always use `addEventListener` (not direct `onXXX` property assignment) to avoid breaking PeerJS's internal ICE candidate trickle mechanism. Players on enterprise/public Wi-Fi with AP Isolation or mobile data with aggressive DPI may still require a VPN or standard residential Wi-Fi.
